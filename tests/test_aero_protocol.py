...
def test_compute_statistics_aero_protocol(mocker):
    ...
    assert hasattr(res_lazy["RMSE"].data, "dask") or type(res_lazy["RMSE"].data).__module__.startswith(
        "cubed"
...
def test_compute_statistics_dask_enabled(mocker):
    ...
    if hasattr(res, "data"):
        assert hasattr(res.data, "dask") or type(res.data).__module__.startswith("cubed"), "Result must be lazy if the metric preserves it"
